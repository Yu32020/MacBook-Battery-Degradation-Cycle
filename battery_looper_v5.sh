...
need battery || fail "缺少 battery：brew install --cask battery"
need brightness || fail "缺少 brightness：brew install brightness"
need caffeinate || fail "缺少 caffeinate"
if [ "$USE_STRESS_NG" = "1" ]; then need stress-ng || fail "缺少 stress-ng：brew install stress-ng"; fi
...
start_load(){
  local N="$(sysctl -n hw.logicalcpu)"
...
echo "RUNNING"
force_brightness_max
( while :; do brightness "$BRIGHT_TARGET" >/dev/null 2>&1 || true; sleep 10; done ) & echo $! > /tmp/looper_bright_keep.pid
start_load
round=0
